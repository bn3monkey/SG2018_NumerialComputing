...
int main(void) {
    ...
    cl_kernel kernel[2];
	cl_mem  buffer_A_GPU, buffer_B_GPU, buffer_C_GPU, 
			buffer_A_CPU, buffer_B_CPU, buffer_C_CPU;
    ...
    work_group_size_GPU = 128; // What would happen if it is 2, 4, 8, 16, 32, 64, 128, 512 or 1024?
	work_group_size_CPU = 128; // What would happen if it is 2, 4, 8, 16, 32, 64, 128, 512 or 1024?
	...
	errcode_ret = clGetDeviceIDs(platform[INDEX_GPU], CL_DEVICE_TYPE_GPU, 1, &devices[INDEX_GPU], NULL);
	...
	errcode_ret = clGetDeviceIDs(platform[INDEX_CPU], CL_DEVICE_TYPE_CPU, 1, &devices[INDEX_CPU], NULL);
    CHECK_ERROR_CODE(errcode_ret);
	...
	context[INDEX_GPU] = clCreateContext(NULL, 1, &devices[INDEX_GPU], NULL, NULL, &errcode_ret);
	CHECK_ERROR_CODE(errcode_ret);
	context[INDEX_CPU] = clCreateContext(NULL, 1, &devices[INDEX_CPU], NULL, NULL, &errcode_ret);
	CHECK_ERROR_CODE(errcode_ret);
	...
	cmd_queues[INDEX_GPU] = clCreateCommandQueue(context[INDEX_GPU], devices[INDEX_GPU], CL_QUEUE_PROFILING_ENABLE, &errcode_ret);
	CHECK_ERROR_CODE(errcode_ret);
    /* Create a command queue for the CPU device. */
	cmd_queues[INDEX_CPU] = clCreateCommandQueue(context[INDEX_CPU], devices[INDEX_CPU], CL_QUEUE_PROFILING_ENABLE, &errcode_ret);
	CHECK_ERROR_CODE(errcode_ret);
	...
	program[INDEX_GPU] = clCreateProgramWithSource(context[INDEX_GPU], 1, (const char **)&prog_src.string, &prog_src.length, &errcode_ret);
	CHECK_ERROR_CODE(errcode_ret);
	program[INDEX_CPU] = clCreateProgramWithSource(context[INDEX_CPU], 1, (const char **)&prog_src.string, &prog_src.length, &errcode_ret);
	CHECK_ERROR_CODE(errcode_ret);
	...
	errcode_ret = clBuildProgram(program[INDEX_GPU], 1, &devices[INDEX_GPU], NULL, NULL, NULL);
	CHECK_ERROR_CODE(errcode_ret);
	if (errcode_ret != CL_SUCCESS) {
		print_build_log(program[INDEX_GPU], devices[INDEX_GPU], "GPU");
		exit(-1);
	}
	errcode_ret = clBuildProgram(program[INDEX_CPU], 1, &devices[INDEX_CPU], NULL, NULL, NULL);
	CHECK_ERROR_CODE(errcode_ret);
    if (errcode_ret != CL_SUCCESS) {
		print_build_log(program[INDEX_CPU], devices[INDEX_CPU], "CPU");
        exit(-1);
    }
	...
	kernel[INDEX_GPU] = clCreateKernel(program[INDEX_GPU], KERNEL_NAME, &errcode_ret);
	CHECK_ERROR_CODE(errcode_ret);
	kernel[INDEX_CPU] = clCreateKernel(program[INDEX_CPU], KERNEL_NAME, &errcode_ret);
	CHECK_ERROR_CODE(errcode_ret);
	...
	buffer_A_GPU = clCreateBuffer(context[INDEX_GPU], CL_MEM_READ_ONLY, sizeof(float)*n_elements, NULL, &errcode_ret);
    CHECK_ERROR_CODE(errcode_ret);
    
	buffer_B_GPU = clCreateBuffer(context[INDEX_GPU], CL_MEM_READ_ONLY, sizeof(float)*n_elements, NULL, &errcode_ret);
    CHECK_ERROR_CODE(errcode_ret);
    
	buffer_C_GPU = clCreateBuffer(context[INDEX_GPU], CL_MEM_WRITE_ONLY, sizeof(float)*n_elements, NULL, &errcode_ret);
    CHECK_ERROR_CODE(errcode_ret);
    
	buffer_A_CPU = clCreateBuffer(context[INDEX_CPU], CL_MEM_READ_ONLY, sizeof(float)*n_elements, NULL, &errcode_ret);
	CHECK_ERROR_CODE(errcode_ret);

	buffer_B_CPU = clCreateBuffer(context[INDEX_CPU], CL_MEM_READ_ONLY, sizeof(float)*n_elements, NULL, &errcode_ret);
	CHECK_ERROR_CODE(errcode_ret);

	buffer_C_CPU = clCreateBuffer(context[INDEX_CPU], CL_MEM_WRITE_ONLY, sizeof(float)*n_elements, NULL, &errcode_ret);
    CHECK_ERROR_CODE(errcode_ret);
	...
	printf_KernelWorkGroupInfo(kernel[INDEX_GPU], devices[INDEX_GPU]);
    
	fprintf(stdout, "   [Data Transfer Host to Devece(GPU)] \n");
    
    CHECK_TIME_START;
    // Move the input data from the host memory to the GPU device memory.
    errcode_ret = clEnqueueWriteBuffer(cmd_queues[INDEX_GPU], buffer_A_GPU, CL_FALSE, 0,
                                       sizeof(float)*n_elements, array_A, 0, NULL, NULL);
    CHECK_ERROR_CODE(errcode_ret);
    
    errcode_ret = clEnqueueWriteBuffer(cmd_queues[INDEX_GPU], buffer_B_GPU, CL_FALSE, 0,
	...
	errcode_ret = clSetKernelArg(kernel[INDEX_GPU], 0, sizeof(cl_mem), &buffer_A_GPU);
	CHECK_ERROR_CODE(errcode_ret);
	errcode_ret = clSetKernelArg(kernel[INDEX_GPU], 1, sizeof(cl_mem), &buffer_B_GPU);
	CHECK_ERROR_CODE(errcode_ret);
	errcode_ret = clSetKernelArg(kernel[INDEX_GPU], 2, sizeof(cl_mem), &buffer_C_GPU);
	CHECK_ERROR_CODE(errcode_ret);
    ...
    CHECK_TIME_START;
	errcode_ret = clEnqueueNDRangeKernel(cmd_queues[INDEX_GPU], kernel[INDEX_GPU], 1, NULL,
	...
	printf_KernelWorkGroupInfo(kernel[INDEX_CPU], devices[INDEX_CPU]);

    memset(array_C, 0, sizeof(float)*n_elements); // Erase the GPU results.
    
	fprintf(stdout, "   [Data Transfer Host to Device(CPU)] \n");

	CHECK_TIME_START;
	// Move the input data from the host memory to the GPU device memory.
	errcode_ret = clEnqueueWriteBuffer(cmd_queues[INDEX_CPU], buffer_A_CPU, CL_FALSE, 0,
		sizeof(float)*n_elements, array_A, 0, NULL, NULL);
	CHECK_ERROR_CODE(errcode_ret);

	errcode_ret = clEnqueueWriteBuffer(cmd_queues[INDEX_CPU], buffer_B_CPU, CL_FALSE, 0,
		sizeof(float)*n_elements, array_B, 0, NULL, NULL);
	CHECK_ERROR_CODE(errcode_ret);
	...
	// queued commands in command_queue have been processed and completed. clFinish is also
	// a synchronization point.
	clFinish(cmd_queues[INDEX_CPU]); // What if this line is removed?
	CHECK_TIME_END(compute_time);
	CHECK_ERROR_CODE(errcode_ret);
	...
	errcode_ret = clSetKernelArg(kernel[INDEX_CPU], 0, sizeof(cl_mem), &buffer_A_CPU);
	CHECK_ERROR_CODE(errcode_ret);
	errcode_ret = clSetKernelArg(kernel[INDEX_CPU], 1, sizeof(cl_mem), &buffer_B_CPU);
	CHECK_ERROR_CODE(errcode_ret);
	errcode_ret = clSetKernelArg(kernel[INDEX_CPU], 2, sizeof(cl_mem), &buffer_C_CPU);
    CHECK_ERROR_CODE(errcode_ret);
    ...
    CHECK_TIME_START;
	errcode_ret = clEnqueueNDRangeKernel(cmd_queues[INDEX_CPU], kernel[INDEX_CPU], 1, NULL,
    ...
    clReleaseMemObject(buffer_A_GPU);
    clReleaseMemObject(buffer_B_GPU);
    clReleaseMemObject(buffer_C_GPU);
	clReleaseMemObject(buffer_A_CPU);
	clReleaseMemObject(buffer_B_CPU);
    clReleaseMemObject(buffer_C_CPU);
	clReleaseKernel(kernel[INDEX_GPU]);
	clReleaseKernel(kernel[INDEX_CPU]);
	clReleaseProgram(program[INDEX_GPU]);
	clReleaseProgram(program[INDEX_CPU]);
    clReleaseCommandQueue(cmd_queues[INDEX_GPU]);
    clReleaseCommandQueue(cmd_queues[INDEX_CPU]);
	clReleaseContext(context[INDEX_GPU]);
	clReleaseContext(context[INDEX_CPU]);
